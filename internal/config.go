...
import (
	...
	// "internal"
	"github.com/hellolib/config/watcher"
...
func NewConfig(ns []string, sources ...Source) (config Config, err error) {
	defer internal.TimeoutGuardWithFunc(
		30*time.Second, nil, nil,
		func(...interface{}) { log.Fatalf("Create config object with source %#v, ns = %v timeout", sources, ns) },
	)()
...
func (s *ConfigImpl) AppendPrefix(prefix []string) error {
	defer internal.TimeoutGuardWithFunc(30*time.Second, nil, nil, func(...interface{}) {
	...
	for level := range s.source {
		if val, err := s.source[level].Sync(); err != nil {
			...
			for k, v := range val {
				...
				s.cache.Set(k, ValueNode{level, v})
			}
...
func (s *ConfigImpl) GetByPrefix(prefix string) map[string]string {
	...
	for k, v := range s.cache.MatchByPrefix(strings.ToLower(prefix)) {
